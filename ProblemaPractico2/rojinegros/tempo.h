...
struct node *remove_balance(struct node *root, int dir, int *done){
  ...
  /* En el caso de tener un hermano rojo*/
  if (is_red(s)){
    /*Hacemos una rotacion simple y reasignamos valores */
    root = single(root, dir);
    s = p->link[!dir];
  }
  /*Vemos si s tiene hijos*/
  if (s != NULL){
    if (!is_red(s->link[0]) && !is_red(s->link[1])){
      /*Si tiene los dos hijos negros hacemos lo mismo que en balancea pasado*/
      if (is_red(p)){
        *done = 1;
      }
      p->red = 0;
